...
# README updates land on this branch via a PR instead of a direct commit to
# the trigger branch, so a user's local clone never diverges from origin
# just because the bot ran. Reused (force-reset to the trigger branch's
# head) across runs rather than creating a new branch every time.
BOT_BRANCH_NAME = "bot/readme-update"
PR_TITLE = BOT_COMMIT_MARKER
PR_BODY = (
    "Automated README update generated by Auto-README Bot.\n\n"
    "This PR is created and updated automatically on each relevant push — "
    "it won't auto-merge. Review the diff and merge when you're happy with it."
)
